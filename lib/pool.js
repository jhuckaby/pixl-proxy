...
module.exports = Class.create({
	...
	defaultConfig: {
		...
		"degraded_perf_monitor": false,
		"degraded_min_avg_ms": 0,
		"degraded_min_req_sec": 0,
		"degraded_min_uptime_sec": 0
	},
	...
	startup: function() {
		...
		// setup async queue
		this.use_queue = this.config.get('use_queue');
		if (this.use_queue) {
			this.queue = async.queue( this.dequeue.bind(this), this.stats.max_executing );
		}
		
		// degraded perf monitor
		this.degraded_perf_state = false;
		if (this.config.get('degraded_perf_monitor')) {
			this.degraded_perf_monitor = true;
			this.degraded_min_avg_ms = this.config.get('degraded_min_avg_ms');
			this.degraded_min_req_sec = this.config.get('degraded_min_req_sec');
			this.degraded_min_uptime_sec = this.config.get('degraded_min_uptime_sec');
		}
	...
	handle_request: function(args, callback) {
		...
		// if pool is setup for queuing, send http response now
		if (this.use_queue) {
			...
			// send request immediately, do not queue
			if (this.stats.num_executing >= this.stats.max_executing) {
				// reject request, we are full
				var err_msg = "Maximum concurrent request limit reached: " + this.stats.num_executing + " requests executing";
				this.logError(429, err_msg, {
					uri: request.url,
					method: request.method,
					req_headers: request.headers
	...
	dequeue: function(task, callback) {
		...
		if (counters.requests && this.stats.max_per_sec && (counters.requests >= this.stats.max_per_sec)) {
			var err_msg = "Request rate has reached max limit: " + Math.floor(counters.requests + 1) + "/sec";
			
			if (!counters.throttle_alert) {
				// only log once per sec
				this.perf.count('throttle_alert', 1);
				this.logError('throttle', err_msg);
			}
		...
		if (this.stats.num_pending) this.stats.num_pending--;
		...
		// possibly add X-Degraded header for slow perf
		if (this.degraded_perf_state) {
			raw_headers['X-Degraded'] = 'true';
		}
		...
		if (this.config.get('preserve_host') && request.headers['host']) {
		...
		this.request[req_func]( url, opts, function(err, resp, data, perf) {
			...
			// mark queue task as complete, if applicable
			if (callback) callback();
	...
	tick: function() {
		...
		metrics.cpu_pct = this.proxy.cpu_pct || 0;
		
		// check for degraded state
		if (this.degraded_perf_monitor) {
			this.degraded_perf_state = !!(
				(metrics.averages.total >= this.degraded_min_avg_ms) && 
				(metrics.counters.requests >= this.degraded_min_req_sec) && 
				((Tools.timeNow(true) - this.server.started) >= this.degraded_min_uptime_sec)
			);
			if (this.degraded_perf_state) metrics.degraded = true;
		}
